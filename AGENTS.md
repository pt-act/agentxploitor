...
### 3. Non-Negotiable Directives: The Hard Rules

These principles are not suggestions. They are inviolable laws of this project.

- **Code Quality:** All frontend component files **must remain under 400 lines**. If a file approaches this limit, you must pause, refactor it into smaller, single-responsibility components, and document the new structure. No exceptions.
...
3. AI Agent updates the memory bank logs.md
...
- **Log progress to `logs.md` incrementally** — don't wait until session end
...
- **Size limit**: Max 1000 lines. Rotate to `logs-02.md`, `logs-03.md`, etc.
- Add entries at TOP (newest first)

**Threads/logs:**
- Create the `threads/` folder in the memory bank structure
- To be used by the human to preserve summaries
---
...
3. **Continuous Quality Consciousness**
   - Frontend Component size enforcement (<400 lines): Automated validation of our principles
...
**Starting a session:**
- If `AGENTS.md` is already loaded by the environment, go straight to `memory_bank/`
